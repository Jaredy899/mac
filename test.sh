...
run_brew_manager() {
    if [ -f "$GITPATH/homebrew_scripts/brew_manager.sh" ]; then
        ...
        sh "$GITPATH/homebrew_scripts/brew_manager.sh"
    else
        echo "Running brew_manager.sh from GitHub..."
        sh -c "$(curl -fsSL $GITHUB_BASE_URL/homebrew_scripts/brew_manager.sh)"
    fi
}
...
run_dock_manager() {
    if [ -f "$GITPATH/dock_scripts/dock_manager.sh" ]; then
        ...
        sh "$GITPATH/dock_scripts/dock_manager.sh"
    else
        echo "Running dock_manager.sh from GitHub..."
        sh -c "$(curl -fsSL $GITHUB_BASE_URL/dock_scripts/dock_manager.sh)"
    fi
}
...
run_myzsh() {
    if [ -f "$GITPATH/myzsh/myzsh.sh" ]; then
        ...
        sh "$GITPATH/myzsh/myzsh.sh"
    else
        echo "Running myzsh.sh from GitHub..."
        sh -c "$(curl -fsSL $GITHUB_BASE_URL/myzsh/myzsh.sh)"
    fi
}
